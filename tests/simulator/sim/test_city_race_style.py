...
def test_city_race_traces_are_thicker_and_prediction_visible() -> None:
    assert PAST_TRACE_WIDTH >= 2.5
    assert PREDICTED_TRACE_WIDTH >= 2.0


def test_city_default_horizon_is_three_seconds() -> None:
    assert DEFAULT_CITY_HORIZON_STEP_COUNT == 60
    assert abs(DEFAULT_CITY_HORIZON_DT - 0.05) < 1e-12
    assert DEFAULT_CITY_HORIZON_STEP_COUNT * DEFAULT_CITY_HORIZON_DT == 3.0
...
def test_path_following_mpc_config_uses_city_defaults_without_yaml() -> None:
    ...
    assert city_cfg.horizon_step_count == 60
...
def test_city_map_yaml_declares_long_horizon() -> None:
    ...
    for name in ("city.yml", "city_mpc_preview.yml"):
        ...
        assert int(horizon["step_count"]) >= 60
        assert float(horizon["dt"]) * int(horizon["step_count"]) >= 3.0
