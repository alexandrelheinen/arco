"""
SST planner on a 2-D continuous environment with scattered obstacles.

Demonstrates the Sparse Stable Trees (SST) planner.  Unlike RRT*, SST
maintains only a sparse representative subset of nodes, which produces a
compact exploration tree at the cost of slightly lower optimality.  The
optional ``--tree`` flag renders the active tree for comparison.
...
def main(save_path: str | None = None, draw_tree: bool = False) -> None:
    ...
    logger.info("Active tree size: %d nodes", len(tree_nodes))
    ...
    fig, ax = draw_occupancy(
        ...
        title=f"SST — {int(_cfg['max_sample_count'])} samples",
    )
    ...
    if save_path is not None:
        ...
        logger.info("Saved SST example to %s", save_path)
    else:
        plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Render the SST active tree.",
    )
